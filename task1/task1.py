...
def insert_data(db, data):
    cursor = db.cursor()
    cursor.executemany("""
        INSERT INTO first (name, street, city, zipcode, floors, year, parking, prob_price, views)
        VALUES(
...
def get_top_by_views(db, limit):
    cursor = db.cursor()
    result = cursor.execute("SELECT * FROM first ORDER BY views DESC LIMIT ?", [limit])
    items = []
